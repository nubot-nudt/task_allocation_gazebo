...
Ball::update(BallObject & teammates_ball,bool is_valid)
{
   ...
   // ROS_INFO("own_ball_ distance:%f",own_ball_.getRealLocation().radius_);
                       ...
                       std::  vector<ros::Time> & _count_time, int & _nums_predict_errors)
{
    ...
    _ball_info[_ball_info.size()-1].setVelocity(ball_global_vec);
    _ball_info[_ball_info.size()-1].setVelocityKnown(ball_vec_known);
